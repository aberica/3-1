...
public:
  ...
  ///////////////////////////////////////////////////////////////////
  std::list<unsigned int> m_lru; // 0 : LRU, end-1 : MRU
};
...
public:
  ...
  bool access(addr_t address, int access_type, int function);
  void print_stats();
  void dump_tag_store(bool is_file);  // false: dump to stdout, true: dump to a file

  addr_t get_victim_address();         // - added for part3
  bool get_victim_dirty();             // - added for part3
  int get_line_size();                 // - added for part3

private:
  std::string m_name;     // cache name
  int m_num_sets;         // number of sets
  int m_line_size;        // cache line size
  addr_t victim_address; // evicted address      - added for part3
  bool victim_dirty;     // is evicted req dirty - added for part3
