...
void memory_hierarchy_c::init(config_c& config) {
  ...
  m_l1u_cache = new cache_c("L1_CACHE", MEM_L1, config.get_l1d_size() / (config.get_l1d_assoc() * config.get_l1d_line_size()), config.get_l1d_assoc(), config.get_l1d_line_size(), config.get_l1d_latency());
  m_l1i_cache = new cache_c("L1I_CACHE", MEM_L1, config.get_l1i_size() / (config.get_l1i_assoc() * config.get_l1i_line_size()), config.get_l1i_assoc(), config.get_l1i_line_size(), config.get_l1i_latency());
  m_l1d_cache = new cache_c("L1D_CACHE", MEM_L1, config.get_l1d_size() / (config.get_l1d_assoc() * config.get_l1d_line_size()), config.get_l1d_assoc(), config.get_l1d_line_size(), config.get_l1d_latency());
  m_l2_cache = new cache_c("L2_CACHE", MEM_L2, config.get_l2_size() / (config.get_l2_assoc() * config.get_l2_line_size()), config.get_l2_assoc(), config.get_l2_line_size(), config.get_l2_latency());
  ...
  if (config.get_mem_hierarchy() == static_cast<int>(Hierarchy::DRAM_ONLY)) {
    ...
    delete m_l2_cache;
    m_l2_cache = new cache_c("L1_CACHE", MEM_L1, config.get_l2_size() / (config.get_l2_assoc() * config.get_l2_line_size()), config.get_l2_assoc(), config.get_l2_line_size(), config.get_l2_latency());
    m_l2_cache->configure_neighbors(nullptr, nullptr, nullptr, m_dram);
    m_dram->configure_neighbors(m_l2_cache);
  } else if (config.get_mem_hierarchy() == static_cast<int>(Hierarchy::MULTI_LEVEL_UNIFIED)) {
    ...
    m_dram->configure_neighbors(m_l2_cache);
  } else if (config.get_mem_hierarchy() == static_cast<int>(Hierarchy::MULTI_LEVEL_SEPARATED)) {
  ...
  m_l1u_cache->set_mem_hierarchy(this);
  m_l1i_cache->set_mem_hierarchy(this);
  m_l1d_cache->set_mem_hierarchy(this);
  m_l2_cache->set_mem_hierarchy(this);
}
...
bool memory_hierarchy_c::access(addr_t address, int access_type) {
  ...
  if (m_config.get_mem_hierarchy() == static_cast<int>(Hierarchy::DRAM_ONLY)) {
    ...
    m_l2_cache->access(req);
  } else if (m_config.get_mem_hierarchy() == static_cast<int>(Hierarchy::MULTI_LEVEL_UNIFIED)) {
    m_l1u_cache->access(req);
  } else if (m_config.get_mem_hierarchy() == static_cast<int>(Hierarchy::MULTI_LEVEL_SEPARATED)) {
    if (access_type == 0 || access_type == 1) m_l1d_cache->access(req);
    else m_l1i_cache->access(req);
  }
  return true;
}
...
void memory_hierarchy_c::free_mem_req(mem_req_s* req) {
  ...
  int debug_size1 = m_in_flight_reqs.size();

  auto& vv = m_in_flight_reqs;
  vv.erase(std::remove(vv.begin(), vv.end(), req), vv.end());

  int debug_size2 = m_in_flight_reqs.size();
...
void memory_hierarchy_c::process_done_req() {
  ...
  for (auto itr = m_in_flight_reqs.begin(); itr != m_in_flight_reqs.end(); ) {
    auto req = *itr;
    if (req->m_done) {
      free_mem_req(req);
      push_done_req(req);
...
bool memory_hierarchy_c::is_wb_done() {
  ...
  ////////////////////////////////////////////////////////////////////
  return m_l1d_cache->m_in_flight_wb_queue->empty() && m_l1i_cache->m_in_flight_wb_queue->empty()
   && m_l1u_cache->m_in_flight_wb_queue->empty() &&  m_l2_cache->m_in_flight_wb_queue->empty();
}
...
std::vector<mem_req_s*> memory_hierarchy_c::get_in_flight_reqs() { return m_in_flight_reqs; }