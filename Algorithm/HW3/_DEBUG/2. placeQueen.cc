...
void recursiveBacktracking(vector<vector<int>>& board, vector<int>& p_max, int row, int col, int p, int& count) {
    int n = board.size();
    if (p >= n) {
        count++;
        return;
    ...
    for (int j = col; j < n; j++) {
        for (int i = 0; i < n; i++) {
            ...
            if (board[i][j] == 0) {
                ...
                if (i+2 >= n) recursiveBacktracking(new_board, p_max, 0, j+1, p+1, count);
                else recursiveBacktracking(new_board, p_max, i+2, j, p+1, count);
            }
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (version == 1) {
        ...
        int count = 0;
        recursiveBacktracking(board, p_max,  0, 0, 0, count);
        outputFile << count << endl;
    }
    else {
