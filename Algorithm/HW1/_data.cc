...
int main() {
	...
	// one
	for(int i = 1; i <= MAX; i++) num[i] = 1;
	ofs.open("./output/one.out");
	ofs << "N RS DS\n"; 
	for(N = 10000; N <= MAX; N += 10000){
		cout<<"one : "<<N<<'\n';
		ofs << N << ' ';
		M = int(N*0.4);

		start = clock();
		int rs1 = Randomized_select(num, 1, N, M);
		finish = clock();
		ofs << (double)(finish - start) / CLOCKS_PER_SEC << ' ';

		start = clock();
		int rs2 = Deterministic_select(num, 1, N, M);
		finish = clock();
		ofs << (double)(finish - start) / CLOCKS_PER_SEC << ' ';
	...
	// random
	for(int i = 1; i <= MAX; i++) num[i] = rand()%MAX;
	ofs.open("./output/random.out");
	ofs << "N RS DS\n"; 
	for(N = 10000; N <= MAX; N += 10000){
		cout<<"random : "<<N<<'\n';
		ofs << N << ' ';
