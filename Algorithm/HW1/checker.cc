...
int main(int argc, char** argv) {
	string inp = argv[1], out = argv[2];
	ifstream ifs1; ifs1.open(inp);
    ifstream ifs2; ifs2.open(out);

    ifs1 >> N >> M;
	for(int i = 1; i <= N; i++) ifs1 >> num[i];

    int check_ans = Deterministic_select(num, 1, N, M);
    int out_ans; ifs2 >> out_ans;
    
    if(check_ans == out_ans) cout << 1 << '\n';
    else cout << 0 << '\n';
